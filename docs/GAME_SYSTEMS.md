...
The game advances one day at a time through the Node command layer in `server/gameCommands.js`. Each day follows this sequence:

```
advanceOneDay(game)
|-- Is there a match today?
|   |-- YES -> simulate scheduled fixtures and apply results
|   |          |-- Update standings
|   |          |-- Update player condition and form
|   |          `-- Generate match result news/articles
|   `-- NO  -> apply daily recovery/training effects
|-- Generate contextual inbox/news events where applicable
`-- Advance the game clock by one day
```
...
Traits are automatically computed from a player's attributes and position by the web game generation helpers in `server/gameFactory.js`.
...
The inbox system provides contextual communication from in-game characters. Messages are generated by the Node command layer in `server/`.
...
The news system generates public-facing articles about league events, displayed in the News tab. Articles are generated by the Node command layer in `server/`.
...
```
createGameState(startupOptions)
|-- Load names definition
|-- Load country/world definition
|-- Select country and fictional clubs
|-- For each club:
...
|-- Generate league schedule from format_code
...
The main external JSON files can customize generation:
- `default_names.json` - Name pools keyed by ISO alpha-2 country code
- `default_world.json` - Playable countries, league formats, fictional clubs, and squad strength profiles
- `default_teams.json` - Legacy fallback team templates for simple generated worlds

If files are not found or have parse errors, the Node generator falls back to bundled defaults.
